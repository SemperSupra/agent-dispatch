// U2 bridge PID1: immutable Ubuntu userspace + writable scratch + explicit resolver bind mount.
typedef unsigned long usize; typedef unsigned long long u64;
...
static void fail(u64 code){char out[96];usize at=0;at=append_lit(out,at,"FIRECRACKER_U2_ERROR code=");at=append_dec(out,at,code);out[at++]='\n';sc3(1,1,(long)out,(long)at);reboot_guest();}
static int wait_dev(const char*path){struct timespec{long tv_sec;long tv_nsec;}nap={0,10*1000*1000};for(int i=0;i<400;i++){long fd=sc3(2,(long)path,0,0);if(fd>=0){sc1(3,fd);return 1;}sc2(35,(long)&nap,0);}return 0;}
static int copy_file(const char*src,const char*dst){
    const long O_RDONLY=0,O_WRONLY=1,O_CREAT=0100,O_TRUNC=01000;long in=sc3(2,(long)src,O_RDONLY,0);if(in<0)return 0;
    long out=sc3(2,(long)dst,O_WRONLY|O_CREAT|O_TRUNC,0644);if(out<0){sc1(3,in);return 0;}char buf[4096];
...
void _start(void){
    ...
    if(!copy_file("/work/u2.py","/newroot/tmp/u2.py"))fail(5);
    if(!copy_file("/work/resolv.conf","/newroot/tmp/resolv.conf"))fail(6);
    if(sc5(SYS_mount,(long)"/newroot/tmp/resolv.conf",(long)"/newroot/etc/resolv.conf",0,MS_BIND,0)<0)fail(7);
    if(sc1(SYS_chroot,(long)"/newroot")<0)fail(8);if(sc1(SYS_chdir,(long)"/")<0)fail(9);
    sc5(SYS_mount,(long)"proc",(long)"/proc",(long)"proc",0,0);
    long pid=sc1(SYS_fork,0);
    if(pid==0){
        char *argv[]={(char*)"/bin/sh",(char*)"-c",(char*)"python3 /tmp/u2.py",0};
        char *envp[]={(char*)"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",(char*)"HOME=/root",(char*)"TMPDIR=/tmp",(char*)"LANG=C",0};
        sc3(SYS_execve,(long)"/bin/sh",(long)argv,(long)envp);sc1(SYS_exit,111);
    }
    int status=0;long waited=sc4(SYS_wait4,pid,(long)&status,0,0);int code=255;if(waited==pid&&(status&0x7f)==0)code=(status>>8)&0xff;
    char out[96];usize at=0;at=append_lit(out,at,"FIRECRACKER_U2_EXIT code=");at=append_dec(out,at,(u64)code);out[at++]='\n';sc3(1,1,(long)out,(long)at);reboot_guest();
}
