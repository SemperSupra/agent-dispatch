// W1 bridge PID1: mount pinned Ubuntu userspace + scratch, then run warm Python workload.
...
static void fail(u64 code){char out[96];usize at=0;at=lit(out,at,"FIRECRACKER_W1_ERROR code=");at=dec(out,at,code);out[at++]='\n';sc3(1,1,(long)out,(long)at);reboot_guest();}
...
void _start(void){
    ...
    if(pid==0){
        char *argv[]={(char*)"/usr/bin/python3",(char*)"/tmp/w1.py",0};
    ...
    char out[96];usize at=0;at=lit(out,at,"FIRECRACKER_W1_EXIT code=");at=dec(out,at,(u64)code);out[at++]='\n';sc3(1,1,(long)out,(long)at);
    reboot_guest();
}
