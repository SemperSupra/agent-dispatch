// W2 bridge PID1: read-only Ubuntu userspace + read-only payload, then snapshot-clone Python state probe.
...
static void fail(u64 code){char out[96];usize at=0;at=lit(out,at,"FIRECRACKER_W2_ERROR code=");at=dec(out,at,code);out[at++]='\n';sc3(1,1,(long)out,(long)at);reboot_guest();}
...
void _start(void){
    ...
    if(pid==0){
        char *argv[]={(char*)"/usr/bin/python3",(char*)"/tmp/w2.py",0};
    ...
    char out[96];usize at=0;at=lit(out,at,"FIRECRACKER_W2_EXIT code=");at=dec(out,at,(u64)code);out[at++]='\n';sc3(1,1,(long)out,(long)at);
    reboot_guest();
}
