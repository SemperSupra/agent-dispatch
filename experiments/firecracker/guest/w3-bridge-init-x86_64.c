// W3 bridge PID1: read-only Ubuntu userspace + payload, then mandatory post-restore reinitialization probe.
...
static void fail(u64 code){char out[96];usize at=0;at=lit(out,at,"FIRECRACKER_W3_ERROR code=");at=dec(out,at,code);out[at++]='\n';sc3(1,1,(long)out,(long)at);reboot_guest();}
...
void _start(void){
    ...
    long cfd=sc3(SYS_open,(long)"/dev/console",O_RDWR,0);
    ...
    if(sc5(SYS_mount,(long)"/dev/vdb",(long)"/newroot/tmp",(long)"ext4",MS_RDONLY,0)<0)fail(4);
    if(sc1(SYS_chroot,(long)"/newroot")<0)fail(5); if(sc1(SYS_chdir,(long)"/")<0)fail(6);
    sc5(SYS_mount,(long)"proc",(long)"/proc",(long)"proc",0,0);
    long pid=sc1(SYS_fork,0);
    if(pid==0){
        char *argv[]={(char*)"/usr/bin/python3",(char*)"/tmp/w3.py",0};
        char *envp[]={(char*)"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",(char*)"HOME=/root",(char*)"TMPDIR=/tmp",(char*)"LANG=C",(char*)"PYTHONDONTWRITEBYTECODE=1",0};
        sc3(SYS_execve,(long)"/usr/bin/python3",(long)argv,(long)envp); sc1(SYS_exit,111);
    }
    int status=0; long waited=sc4(SYS_wait4,pid,(long)&status,0,0); int code=255;
    if(waited==pid&&(status&0x7f)==0)code=(status>>8)&0xff;
    char out[96];usize at=0;at=lit(out,at,"FIRECRACKER_W3_EXIT code=");at=dec(out,at,(u64)code);out[at++]='\n';sc3(1,1,(long)out,(long)at);
    reboot_guest();
}
