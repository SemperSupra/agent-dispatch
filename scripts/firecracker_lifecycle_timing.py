...
class LifecycleTimer:
    ...
    def add(self, name: str, scope: str, elapsed_ms: float) -> None:
        if scope not in {"portable", "venue"}:
            raise ValueError(f"invalid timing scope: {scope}")
        self._stages.append(
            {
    ...
    def receipt(self) -> dict:
        ...
        portable_ms = round(
            sum(stage["elapsed_ms"] for stage in self._stages if stage["scope"] == "portable"),
            3,
        )
        venue_ms = round(
            sum(stage["elapsed_ms"] for stage in self._stages if stage["scope"] == "venue"),
            3,
        )
        return {
            ...
            "note": (
                "Stage sums exclude untimed Python/control overhead and may overlap only if "
                "a caller explicitly nests stages; current Firecracker probes use non-overlapping stages."
            ),
        }
