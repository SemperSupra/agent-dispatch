#!/usr/bin/env python3
"""W2: characterize duplicated vs regenerated state when reusing one warm snapshot."""
...
SCHEMA="firecracker-w2-snapshot-clone-state/v1"
...
INIT_SOURCE=pathlib.Path("experiments/firecracker/guest/w2-bridge-init-x86_64.c")
GUEST_SOURCE=pathlib.Path("experiments/firecracker/guest/w2-clone-state.py")
READY_RE=re.compile(r"FIRECRACKER_W2_READY session=([0-9a-f]{32}) python=(\S+) pid=(\d+)")
CLONE_RE=re.compile(
    r"FIRECRACKER_W2_CLONE session=([0-9a-f]{32}) py_random=([0-9a-f]{32}) "
    r"urandom=([0-9a-f]{32}) secrets=([0-9a-f]{32})"
)
EXIT_RE=re.compile(r"FIRECRACKER_W2_EXIT code=(\d+)")
...
def _preload_payload(image:pathlib.Path)->dict:
    ...
    cp=subprocess.run(
        [debugfs,"-w","-R",f"write {GUEST_SOURCE.resolve()} /w2.py",str(image)],
        capture_output=True,text=True,timeout=20,check=False,
    )
    return {"ok":cp.returncode==0,"stdout":cp.stdout[-2000:],"stderr":cp.stderr[-2000:]}

def compare_clone_state(a:dict,b:dict)->dict:
    return {
        "pre_snapshot_session_duplicated":a.get("session")==b.get("session"),
        "python_prng_output_duplicated":a.get("py_random")==b.get("py_random"),
        "kernel_urandom_unique":a.get("urandom")!=b.get("urandom"),
        "secrets_token_unique":a.get("secrets")!=b.get("secrets"),
    }
...
def _restore_clone(*,index:int,firecracker:pathlib.Path,snapshot:pathlib.Path,memory:pathlib.Path,work:pathlib.Path)->dict:
    ...
    marker=p3._wait_for_regex(lines,CLONE_RE,5)
    ...
    if marker:
        observed={
            "session":marker.group(1),
            "py_random":marker.group(2),
            "urandom":marker.group(3),
            "secrets":marker.group(4),
        }
    reseed=any(VMFORK_MARKER in line for line in lines)
    return {
        "ok":bool(observed) and not ready_seen and bool(exit_values) and max(exit_values)==0 and rc==0,
        "index":index,
        "firecracker_pid":pid,
        "destination_reemitted_ready":ready_seen,
        "exit_code":rc,
        "observed":observed,
        "vmfork_crng_reseed_log_observed":reseed,
        "snapshot_load_ms":load.get("elapsed_ms"),
        "resume_api_ms":resume.get("elapsed_ms"),
        "resume_to_clone_marker_ms":resume_to_marker_ms,
        "load_start_to_clone_marker_ms":load_to_marker_ms,
        "output_tail":"\n".join(lines[-100:]),
    }
...
def run_probe(label:str)->dict:
    ...
    with tempfile.TemporaryDirectory(prefix="firecracker-w2-") as td:
        w=pathlib.Path(td)
        archive=w/"firecracker.tgz"; extract=w/"extract"; extract.mkdir()
        kernel=w/"vmlinux"; rootfs=w/"ubuntu.squashfs"; payload=w/"payload.ext4"
        init_bin=w/"init"; initrd=w/"initrd.cpio"; config_path=w/"source-config.json"
        source_sock=w/"source.sock"; state_file=w/"snapshot.state"; mem_file=w/"snapshot.mem"
        ...
        root_before=_sha256(rootfs); payload_before=_sha256(payload)
        cfg=f1._build_config(kernel,initrd,config_path)
        cfg["machine-config"]["vcpu_count"]=1; cfg["machine-config"]["mem_size_mib"]=512
        cfg["drives"]=[
        ...
        if not ready:
            p3._terminate_group(source)
            return {"schema":SCHEMA,"authority":AUTHORITY,"result":{"classification":"ORACLE_FAILURE","reason":"source READY not observed"}}
        source_session=ready.group(1)
        ...
        with timer.stage("clone1_restore","portable"):
            clone1=_restore_clone(index=1,firecracker=firecracker,snapshot=state_file,memory=mem_file,work=w)
        clone1_absent=True
        if clone1.get("firecracker_pid"):
            clone1_absent=not pathlib.Path(f"/proc/{clone1['firecracker_pid']}").exists()

        with timer.stage("clone2_restore","portable"):
            clone2=_restore_clone(index=2,firecracker=firecracker,snapshot=state_file,memory=mem_file,work=w)

        root_after=_sha256(rootfs); payload_after=_sha256(payload)
        read_only_images_unchanged=(
            root_before==root_after==um["rootfs_sha256"]
            and payload_before==payload_after
        )
        observed1=clone1.get("observed") or {}
        observed2=clone2.get("observed") or {}
        semantics=compare_clone_state(observed1,observed2)
        session_matches_source=(
            observed1.get("session")==source_session
            and observed2.get("session")==source_session
        )
        distinct_pids=(
        ...
        mechanics_ok=bool(
            ...
            and session_matches_source and distinct_pids
            and read_only_images_unchanged
        )
        return {
            "schema":SCHEMA,
            "authority":AUTHORITY,
            ...
            "result":{
                "classification":"SUPPORTED" if mechanics_ok else "ORACLE_FAILURE",
                "two_sequential_restores_supported":mechanics_ok,
                "source_absent_before_clones":source_absent,
                "clone1_absent_before_clone2":clone1_absent,
                "pre_snapshot_session_matches_both_clones":session_matches_source,
                "distinct_firecracker_pids":distinct_pids,
                "read_only_images_unchanged":read_only_images_unchanged,
            },
            "source":{"firecracker_pid":source_pid,"session":source_session},
            "clone1":clone1,
            "clone2":clone2,
            "state_semantics":semantics,
            "security_interpretation":{
                "vmfork_reseed_observed_both":bool(clone1.get("vmfork_crng_reseed_log_observed") and clone2.get("vmfork_crng_reseed_log_observed")),
                "kernel_backed_randomness_differs":bool(semantics["kernel_urandom_unique"] and semantics["secrets_token_unique"]),
                "application_prng_state_duplicated":semantics["python_prng_output_duplicated"],
                "pre_snapshot_application_session_duplicated":semantics["pre_snapshot_session_duplicated"],
            },
            "snapshot":snapshot,
            "api":{"pause":pause,"create":create,"source_termination":source_term},
            "lifecycle_timing":timer.receipt(),
            "next_gate":"If application state duplicates, reusable warm templates require explicit post-restore reinitialization before credentialed/identity-bearing workloads.",
        }
