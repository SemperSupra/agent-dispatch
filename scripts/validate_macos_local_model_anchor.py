...
RAW_SCHEMA = "macos-local-model-anchor/raw-v3"
VALIDATION_SCHEMA = "macos-local-model-anchor/validation-v3"
...
def check(raw: dict[str, Any]) -> tuple[bool, list[str], list[str]]:
    ...
    generation_args = task.get("generation_args") or []
    if "--single-turn" not in generation_args:
        errors.append("single-turn-lifecycle-missing")
    if "--simple-io" not in generation_args:
        errors.append("simple-io-missing")
    if "--no-show-timings" not in generation_args:
        errors.append("timing-noise-not-disabled")
